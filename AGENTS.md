...
| File | Role |
|------|------|
| `agent-pc/agent.py` | **Single file agent** (~440 lines, v10.3). All logic here. |
...
- **BUG 4**: RESOLVED in practice — iGPU Vulkan offload (9-50s/turn, was 31-629s CPU-only).
- **BUG 5**: Model confuses audio/video extensions. Few-shot examples in system prompt help but aren't 100%.

Bugs 1 (command dedup), 2 (200-char limit) fixed in v10.1; Bug 3 (tool output caps 600/600/800) fixed in v10.2; retry dead-end fixed + multi-model support in v10.3. `tests.sh` timeouts raised (120-300s) with per-test failure guards.
